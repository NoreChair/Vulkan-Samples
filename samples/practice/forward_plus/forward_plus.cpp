...
void GenTestSH(float* pSh) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        *(iter++) = rgbSH[offset + 4]; *(iter++) = rgbSH[offset + 5]; *(iter++) = rgbSH[offset + 6]; *(iter++) = rgbSH[offset + 7];
    }
    *(iter++) = rgbSH[8];
    *(iter++) = rgbSH[17];
    *(iter++) = rgbSH[26];
...
void forward_plus::render(float delta_time) {
    ...
    // HDR process and auto exposure
    {
        ...
        toneMapUniforms.targetLuma = 0.08f;
...

    {
        ...
        commandBuffer.begin_render_pass(renderTarget, renderPass, frameBuffer, zeroClearValue);
